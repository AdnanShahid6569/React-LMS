...
const [valsyll, setValsyll] = useState([]);
...
const delsyll = async (id) =>{
  ...
  return (
    ...
    <Paper sx={{ display: 'flex', justifyContent: "space-between", padding: '20px', background: '#FBFFE4' }}>
        <Typography variant='h4' sx={{ textAlign: 'center' }}>Syllabus List</Typography>
        <Button variant="contained" color="success" onClick={()=>{navigate('/Dashboard/Syllabusadd')}}>Add</Button>
      </Paper>
      <Divider />
          ...
          <tbody>
            {valsyll.map((e,i) => (
              <tr key={e.id} style={{ border: '1px solid #ddd', textAlign:'center'}}>
                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{e.Question}</td>
                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{e.name}</td>
                <td style={{ padding: '10px', border: '1px solid #ddd' }}>{e.class}</td>
                <td style={{textAlign:'center'}} ><Button variant='contained' color='warning'  onClick={() => { delsyll(e.id) }}>Delete</Button></td>
              </tr>
            ))}
          </tbody>
        </table>
